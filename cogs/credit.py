import disnake, requests, random, json, os, shutil
from disnake.ext import commands, tasks
from disnake.ext.commands.core import has_guild_permissions, has_permissions
...
# ===========
    ...
    async def credit(self, ctx, user: Union[disnake.Member, int] = None):
        if user == None:
            user = ctx.author
        ...
        emb.set_thumbnail(url=ctx.guild.me.avatar.url)
        emb.set_footer(text=user,icon_url=user.avatar.url)
    ...
    async def add(self, ctx, user: Union[disnake.Member, int] = None, credit = 0):
        ...
        if type(user).__name__ == "int":
            try:
                ...
                raise disnake.ext.commands.MemberNotFound(user)
        if user == None:
            if lang == "RU":
                nouseremb = disnake.Embed(
                    title='Ошибка!',
            ...
            nouseremb.set_footer(text=ctx.author, icon_url=ctx.author.avatar.url)
            ...
            selfemb.set_footer(text=ctx.author, icon_url=ctx.author.avatar.url)
            ...
            try:
                ...
                if lang == "RU":
                    ...
                    emb.set_footer(text=f'Добавлено юзером {ctx.author}', icon_url=ctx.author.avatar.url)
                elif lang == "EN":
                    emb = disnake.Embed(
                    ...
                    emb.set_footer(text=f'Added by {ctx.author}', icon_url=ctx.author.avatar.url)
                emb.set_thumbnail(url=ctx.guild.me.avatar.url)
                await ctx.reply(embed=emb)
                ...
                emb.set_image(url='https://media.disnakeapp.net/attachments/883779765415337995/896431661632344074/firefox_8oM4iRMc8K.png')
    ...
    async def remove(self, ctx, user: Union[disnake.Member, int] = None, credit = 0):
        credittrigger = False
        lang = self.check_lang(ctx.guild)
        if type(user).__name__ == "int":
            try:
        ...
        if user == None:
            ...
            if lang == "RU":
                ...
                emb.set_footer(text=f'Убрано юзером {ctx.author}', icon_url=ctx.author.avatar.url)
            elif lang == "EN":
                emb = disnake.Embed(
                ...
                emb.set_footer(text=f'Removed by {ctx.author}', icon_url=ctx.author.avatar.url)
            emb.set_thumbnail(url=ctx.guild.me.avatar.url)
            await ctx.reply(embed=emb)
    ...
    async def set(self, ctx, user: Union[disnake.Member, int] = None, credit = 0):
        lang = self.check_lang(ctx.guild)
        if type(user).__name__ == "int":
            try:
                user = await self.bot.fetch_user(user)
        ...
        if user == None:
            ...
            if lang == "RU":
                ...
                emb.set_footer(text=f'Установлено юзером {ctx.author}', icon_url=ctx.author.avatar.url)
            elif lang == "EN":
                emb = disnake.Embed(
                ...
                emb.set_footer(text=f'Set by {ctx.author}', icon_url=ctx.author.avatar.url)
            emb.set_thumbnail(url=ctx.guild.me.avatar.url)
            await ctx.reply(embed=emb)
    ...
    async def lang(self, ctx, lang):
        try:
            ...
            if newlang == "EN":
                ...
                emb.set_thumbnail(url=ctx.guild.me.avatar.url)
                emb.set_footer(text=f'Changed by {ctx.author}', icon_url=ctx.author.avatar.url)
            elif newlang == "RU":
                emb = disnake.Embed(
                    title='Успешно!',
                ...
                emb.set_thumbnail(url=ctx.guild.me.avatar.url)
                emb.set_footer(text=f'Изменено {ctx.author}', icon_url=ctx.author.avatar.url)
            await ctx.reply(embed=emb)
    ...
    async def reset(self, ctx):
        ...
        if ctx.author == ctx.guild.owner:
            ...
            if lang == "RU":
                ...
                emb.set_thumbnail(url=ctx.guild.me.avatar.url)
                emb.set_footer(text=f'Очищено {ctx.author}', icon_url=ctx.author.avatar.url)
                await ctx.reply(embed=emb)
            elif lang == "EN":
                emb = disnake.Embed(
                    title='Success!',
                ...
                emb.set_thumbnail(url=ctx.guild.me.avatar.url)
                emb.set_footer(text=f'Reset by {ctx.author}', icon_url=ctx.author.avatar.url)
                await ctx.reply(embed=emb)
        else: 
            if lang == "RU":
                emb = disnake.Embed(
